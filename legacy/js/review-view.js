...
const locale = currentlang === "english" ? "en" : "nl";
    (() => {
      const refs = {
        ...
        loadMoreButton: document.getElementById("button-view"), // Додано
      };
      ...
      async function fetchReviewData() { // Замінено
        try {
      ...
      function init() { // Додано
        ...
        function toggleComments() {
          if (commentsToShow >= comments.length) {
            ...
            commentsToShow += 10;
            if (commentsToShow >= comments.length) {
              refs.loadMoreButton.textContent = 'Show Less';
            }
          }
