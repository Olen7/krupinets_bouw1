...
const contact = {
  ...
  export function updatePhone(){
    ...
    updatePhoneEl.insertAdjacentHTML("beforeend", `<a href="tel:${contact.phone}" class="address_link lang " data-key="phone">Phone: <span class="foother_desctop_amster ">${contact.phone}</span></a>`)
    localStorage.setItem("phone", contact.phone)
      
  }
  function updateEmail(){
      const updateEmailEl = document.querySelector('#update-email')
      updateEmailEl.insertAdjacentHTML("beforeend", `<a href="mailto:${contact.email}" class="address_link">E-mail: <span class="foother_desctop_amster">${contact.email}</span> </a>`)
      localStorage.setItem("email", contact.email)
    }
    
    function updateAddress() {
        const updateAddressEl = document.querySelector('#update-address')
        updateAddressEl.insertAdjacentHTML("beforeend", ` <a href="https://maps.app.goo.gl/yW653rWedA1cQ6HC8" class="address_link" target="_blank" rel="noopener noreferrer nofollow lang" data-key="address">Address: <span class="foother_desctop_amster">${contact.address},<span class="foother_tab_amster">Amsterdam, Netherlands</span></span></a>`)
